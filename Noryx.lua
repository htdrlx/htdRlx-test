...
-- NORYX CORE SERVICE CACHE (performance / consistency)
local NRX_Services = {}
NRX_Services.ReplicatedStorage = NRX_Services.ReplicatedStorage
NRX_Services.Players = NRX_Services.Players
NRX_Services.Workspace = NRX_Services.Workspace
NRX_Services.RunService = NRX_Services.RunService
NRX_Services.TeleportService = NRX_Services.TeleportService
NRX_Services.HttpService = NRX_Services.HttpService
NRX_Services.TweenService = NRX_Services.TweenService
NRX_Services.StarterGui = NRX_Services.StarterGui
NRX_Services.CollectionService = NRX_Services.CollectionService
NRX_Services.Lighting = NRX_Services.Lighting
NRX_Services.CoreGui = NRX_Services.CoreGui
NRX_Services.VirtualInputManager = NRX_Services.VirtualInputManager
NRX_Services.VirtualUser = NRX_Services.VirtualUser
NRX_Services.Stats = NRX_Services.Stats
NRX_Services.LocalizationService = NRX_Services.LocalizationService
local NRX_Players = NRX_Services.Players
local NRX_LocalPlayer = NRX_Players.LocalPlayer
local NRX_ReplicatedStorage = NRX_Services.ReplicatedStorage
local NRX_Workspace = NRX_Services.Workspace
local NRX_RunService = NRX_Services.RunService
local NRX_TweenService = NRX_Services.TweenService
local NRX_HttpService = NRX_Services.HttpService

  ply = NRX_Players
  plr = NRX_LocalPlayer
  Root = plr.Character.HumanoidRootPart
  replicated = NRX_ReplicatedStorage
  Lv = plr.Data.Level.Value
  TeleportService = NRX_Services.TeleportService
  TW = NRX_TweenService
  Lighting = NRX_Services.Lighting
  Enemies = NRX_Workspace:FindFirstChild("Enemies") or workspace.Enemies
  vim1 = NRX_Services.VirtualInputManager
  vim2 = NRX_Services.VirtualUser
  TeamSelf = plr.Team
  RunSer = NRX_RunService
  Stats = NRX_Services.Stats
...
-- NORYX RESPAWN GUARD: refresh cached character references after respawn
local function NRX_RefreshCharacter()
    ...
    if energy then Energy = energy.Value end
    TeamSelf = plr.Team
end
NRX_RefreshCharacter()
if plr then
    plr.CharacterAdded:Connect(function()
        task.wait(0.15)
        pcall(NRX_RefreshCharacter)
    end)
end

-- NORYX SAFE HELPERS
local function NRX_IsAliveCharacter()
    local char = plr and plr.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    return char ~= nil and hrp ~= nil and hum ~= nil and hum.Health > 0, char, hrp, hum
end

-- Biến chọn mode AI (thay đổi bằng dropdown)
_G.AIMode = "normal" -- Mặc định
HttpService = NRX_HttpService
...
hookfunction(require(NRX_Services.ReplicatedStorage.Effect.Container.Death),function() end)
hookfunction(require(NRX_Services.ReplicatedStorage:WaitForChild("GuideModule")).ChangeDisplayedNPC,function()end)
...
gay = (function()
  ...
  local lighting = NRX_Services.Lighting
...
local Players = NRX_Services.Players
local TS = NRX_Services.TweenService
...
local RunService = NRX_Services.RunService
...
_tp = function(I, lockToken)
    ...
    local tween = NRX_Services.TweenService:Create(HRP, info, { CFrame = I })
...
do
    ...
    if button then
        ...
        local RED = Color3.fromRGB(235, 18, 18)
        local WHITE = Color3.fromRGB(255, 255, 255)

        local function frame(name, size, pos, color, rot, z)
        ...
        outer.Size = UDim2.fromScale(0.90, 0.90)
        ...
        -- Four segmented ring pieces, matching the original circular emblem.
        local ringPieces = {
            {"RingRedTop",    UDim2.fromScale(0.38,0.065), UDim2.fromScale(0.39,0.18), RED,  -18},
            {"RingWhiteTop",  UDim2.fromScale(0.30,0.065), UDim2.fromScale(0.69,0.22), WHITE,  35},
            {"RingRedBottom", UDim2.fromScale(0.36,0.065), UDim2.fromScale(0.35,0.80), RED,   25},
            {"RingWhiteBottom",UDim2.fromScale(0.36,0.065), UDim2.fromScale(0.68,0.80), WHITE,-25},
        }
        for _, d in ipairs(ringPieces) do
            local r = frame(d[1], d[2], d[3], d[4], d[5], 54)
        ...
        -- N: red body with black edge.
        local n = Instance.new("TextLabel")
        n.Name = "N"
        n.BackgroundTransparency = 1
        n.Size = UDim2.fromScale(0.55, 0.66)
        n.Position = UDim2.fromScale(0.07, 0.17)
        n.Font = Enum.Font.GothamBlack
        n.Text = "N"
        n.TextScaled = true
        n.TextColor3 = RED
        n.TextStrokeColor3 = BLACK
        n.TextStrokeTransparency = 0
        n.ZIndex = 58
        n.Parent = logo

        -- X: white body with a thick black outline, slightly offset over N.
        local x = Instance.new("TextLabel")
        x.Name = "X"
        x.BackgroundTransparency = 1
        x.Size = UDim2.fromScale(0.62, 0.64)
        x.Position = UDim2.fromScale(0.40, 0.20)
        x.Font = Enum.Font.GothamBlack
        x.Text = "X"
        x.TextScaled = true
        x.TextColor3 = WHITE
        x.TextStrokeColor3 = BLACK
        x.TextStrokeTransparency = 0
        x.ZIndex = 60
        x.Parent = logo

        -- Small central black separator, keeping the emblem's sharp center.
        local center = frame("Center", UDim2.fromScale(0.065,0.065), UDim2.fromScale(0.55,0.53), BLACK, 0, 61)
...
task.spawn(function()
    ...
    while task.wait(0.75) do
        pcall(function()
            local cg = NRX_Services.CoreGui
            paint(cg)
        end)
        pcall(function()
            paint(NRX_Services.Players.LocalPlayer:FindFirstChildOfClass("PlayerGui") or NRX_Services.Players.LocalPlayer.PlayerGui)
        end)
    end
end)
...
function UpdateOS()
    ...
    local LocalizationService = NRX_Services.LocalizationService
    local Players = NRX_Services.Players
...
task.spawn(function()
    ...
    while task.wait(1) do
        local currentStatus = NRX_Services.Workspace.Map:FindFirstChild("KitsuneIsland") and '✅️' or '❌️'
        if currentStatus ~= previousStatus then
...
task.spawn(function()
    while task.wait(1) do
        local cakePrince = NRX_Services.ReplicatedStorage.Remotes.CommF_:InvokeServer("CakePrinceSpawner")
...
task.spawn(function()
    local previousStatus = ""
    while task.wait(1) do
        local currentStatus = NRX_Services.ReplicatedStorage:FindFirstChild("rip_indra True Form") or NRX_Services.Workspace.Enemies:FindFirstChild("rip_indra") and '✅️' or '❌️'
        if currentStatus ~= previousStatus then
...
task.spawn(function()
    local previousStatus = ""
    while task.wait(1) do
        local currentStatus = NRX_Services.ReplicatedStorage:FindFirstChild("Dough King") or NRX_Services.Workspace.Enemies:FindFirstChild("Dough King") and '✅️' or '❌️'
        if currentStatus ~= previousStatus then
...
task.spawn(function()
    local previousStatus = ""
    while task.wait(1) do
        local currentStatus = (NRX_Services.ReplicatedStorage:FindFirstChild("Diablo") or NRX_Services.ReplicatedStorage:FindFirstChild("Deandre") or NRX_Services.ReplicatedStorage:FindFirstChild("Urban") or NRX_Services.Workspace.Enemies:FindFirstChild("Diablo") or NRX_Services.Workspace.Enemies:FindFirstChild("Deandre") or NRX_Services.Workspace.Enemies:FindFirstChild("Urban")) and '✅️' or '❌️'
        local progress = NRX_Services.ReplicatedStorage.Remotes.CommF_:InvokeServer("EliteHunter","Progress")
        if currentStatus ~= previousStatus then
...
task.spawn(function()
    while task.wait(1) do
        local moonTextureId = NRX_Services.Lighting.Sky.MoonTextureId
...
task.spawn(function()
    ...
    while task.wait(1) do
        ...
        if NRX_Services.ReplicatedStorage.Remotes.CommF_:InvokeServer("LegendarySwordDealer", "1") then
            swordStatus = "Shisui"
        elseif NRX_Services.ReplicatedStorage.Remotes.CommF_:InvokeServer("LegendarySwordDealer", "2") then
            swordStatus = "Wando"
        elseif NRX_Services.ReplicatedStorage.Remotes.CommF_:InvokeServer("LegendarySwordDealer", "3") then
            swordStatus = "Saddi"
        end
...
task.spawn(function()
    while task.wait(1) do
        StatusBone:SetDesc("You Have: " .. tostring(NRX_Services.ReplicatedStorage.Remotes.CommF_:InvokeServer("Bones", "Check")) .. " Bones")
    end
end)
...
Tabs.Main:AddButton({
    ...
    Callback = function()
        if bugContent == "" or bugContent == nil then
            NRX_Services.StarterGui:SetCore("SendNotification", {
                Title = "Lỗi",
        ...
        local HttpService = NRX_Services.HttpService
...
Tabs.VS:AddButton({
    ...
    Callback = function()
        local Players = NRX_Services.Players
        local ReplicatedStorage = NRX_Services.ReplicatedStorage
        local player = Players.LocalPlayer
...
Tabs.VS:AddButton({
    ...
    Callback = function()
        local P   = NRX_Services.Players.LocalPlayer
...
local Players = NRX_Services.Players
local RS = NRX_Services.ReplicatedStorage
...
task.spawn(function()
    while task.wait() do -- Dùng task.wait() thay vì wait() để tối ưu hơn
        if _G.AutoFarmChest then
            pcall(function()
                local CollectionService = NRX_Services.CollectionService
                local Players = NRX_Services.Players
                local Player = Players.LocalPlayer
    ...
    while task.wait(Sec) do
        if _G.AutoBerryH then
            ...
            if #BerryBush == 0 then
                ...
                local TeleportService = NRX_Services.TeleportService
                local ServerList = {}
                
                -- Lấy danh sách server
                local Success, Error = pcall(function()
                    ServerList = NRX_Services.HttpService:JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"))
                end)
...
local function HopServer()
	local Http = NRX_Services.HttpService
	local TPS = NRX_Services.TeleportService
                ...
                if boss then
                    ...
                    for _, v in pairs(data.data) do
                        if v.playing < v.maxPlayers then
                            NRX_Services.TeleportService:TeleportToPlaceInstance(game.PlaceId, v.id, player)
                            break
                        end
                    end
                end
...
local function sendSkillKey(skillKey)
    local virtualInputManager = NRX_Services.VirtualInputManager
...
task.spawn(function()
  ...
  local function handleEnemySpawns()
    for _, v in pairs(NRX_Services.Workspace["_WorldOrigin"].EnemySpawns:GetChildren()) do
...
P = NRX_Services.Players
R = NRX_Services.ReplicatedStorage
L = P.LocalPlayer
...
-- ========== MAIN SCRIPT ==========
task.spawn(function()
    local plr = NRX_Services.Players.LocalPlayer
        ...
        pcall(function()
            if _G.AutoHopServer then
                ...
                if elapsed >= 1800 then -- 1800s = 30 phút
                    ...
                    NRX_Services.TeleportService:Teleport(game.PlaceId, game.Players.LocalPlayer)
                end
            else
                _G.HopTimer = nil
            end
...
Players = NRX_Services.Players
VirtualUser = NRX_Services.VirtualUser
...
HttpService = NRX_Services.HttpService
CoreGui = NRX_Services.CoreGui
...
CollectionService = NRX_Services.CollectionService
...
Players2 = NRX_Services.Players
LocalPlayer2 = Players2.LocalPlayer
Workspace2 = NRX_Services.Workspace
FishReplicated2 = NRX_Services.ReplicatedStorage:WaitForChild("FishReplicated")
FishingRequest2 = FishReplicated2:WaitForChild("FishingRequest")
Config2 = require(FishReplicated2.FishingClient.Config)
GetWaterHeight2 = require(NRX_Services.ReplicatedStorage.Util.GetWaterHeightAtLocation)
...
Players3 = NRX_Services.Players
LocalPlayer3 = Players3.LocalPlayer
ReplicatedStorage3 = NRX_Services.ReplicatedStorage
...
ReplicatedStorage4 = NRX_Services.ReplicatedStorage
    ...
    Callback = function()
        task.spawn(function()
            pcall(function()
                ...
                local TweenService = NRX_Services.TweenService
                ...
                local noclip = NRX_Services.RunService.Stepped:Connect(function()
                ...
                if (hrp.Position - doorCFrame.Position).Magnitude <= 20 then
                    ...
                    NRX_Services.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", insideVector)
                end
            end)
        end)
    end
      ...
      if _G.Complete_Trials then
	    if tostring(plr.Data.Race.Value) == "Fishman" then
	      if GetSeaBeastTrial() then            
            repeat task.wait()
              task.spawn(function()_tp(CFrame.new(GetSeaBeastTrial().HumanoidRootPart.Position.X,NRX_Services.Workspace.Map["WaterBase-Plane"].Position.Y + 300,GetSeaBeastTrial().HumanoidRootPart.Position.Z))end)
...
checkQuesta=function()local a={[1]={["Context"]="Check"}}local b=nil;pcall(function()local c={[1]={["Context"]="RequestQuest"}}NRX_Services.ReplicatedStorage:WaitForChild("Modules"):WaitForChild("Net"):WaitForChild("RF/DragonHunter"):InvokeServer(unpack(c))end)local d,e=pcall(function()b=NRX_Services.ReplicatedStorage:WaitForChild("Modules"):WaitForChild("Net"):WaitForChild("RF/DragonHunter"):InvokeServer(unpack(a))end)local f=false;local g;local h;local i;if b then if b.Text then f=true;local j=b.Text;if string.find(tostring(j),"Defeat")then i=1;g=string.sub(tostring(j),8,9)g=tonumber(g)local k={"Hydra Enforcer","Venomous Assailant"}for l,m in pairs(k)do if string.find(j,m)then h=m;break end end elseif string.find(tostring(j),"Destroy")then g=10;i=2;h=nil end end end;return f,h,g,i end
BackTODoJo=function()for a,b in pairs(NRX_Services.Players.LocalPlayer.PlayerGui.Notifications:GetChildren())do if b.Name=="NotificationTemplate"then if string.find(b.Text,"Head back to the Dojo to complete more tasks")then return true end end end;return false end
...
Players = NRX_Services.Players
RunService = NRX_Services.RunService
LocalPlayer = Players.LocalPlayer
...
task.spawn(function()
    local ply = NRX_Services.Players
...
local function InfiniteSoruLoop()
    while getgenv().InfiniteSoru do
        local character = NRX_Services.Players.LocalPlayer.Character
...
task.spawn(function()
    pcall(function()
        NRX_Services.RunService.Stepped:Connect(function()
...
lp = NRX_Services.Players.LocalPlayer
replicated = NRX_Services.ReplicatedStorage
TweenService = NRX_Services.TweenService
RunService = NRX_Services.RunService
...
HTTPService = NRX_Services.HttpService
TeleportService = NRX_Services.TeleportService
StatsService = NRX_Services.Stats
...
Tabs.Misc:AddButton({
    ...
    Callback = function(Value)  -- Value nil/false, không ảnh hưởng
        ...
        for i, code in ipairs(codes) do
            ...
            local success, err = pcall(function()
                NRX_Services.ReplicatedStorage
...
Callback = function(Value)
  ...
  if _G.WalkWater_Part then
    NRX_Services.Workspace.Map["WaterBase-Plane"].Size = Vector3.new(1000, 112, 1000)
  else
    NRX_Services.Workspace.Map["WaterBase-Plane"].Size = Vector3.new(1000, 80, 1000)
  end
end
})
...
Players = NRX_Services.Players
RS = NRX_Services.ReplicatedStorage
RunService = NRX_Services.RunService
...
do
    ...
    local RunService = NRX_Services.RunService
    local ReplicatedStorage = NRX_Services.ReplicatedStorage
    local Workspace = NRX_Services.Workspace
    ...
    local function SendHits()
        ...
        -- Gửi đúng 2 mục tiêu trong một nhịp RegisterHit.
        -- Ưu tiên 2 mob gần nhau/gần người chơi nhất.
        local candidates = {}
        for _, enemy in ipairs(enemyCache) do
            local hrp = enemy:FindFirstChild("HumanoidRootPart")
            local hum = enemy:FindFirstChildOfClass("Humanoid")
            if hrp and hum and hum.Health > 0 then
                candidates[#candidates + 1] = { enemy = enemy, hrp = hrp, dist = (hrp.Position - rootPart.Position).Magnitude }
            end
        end
        ...
        for _, item in ipairs(candidates) do
            ...
            local enemy, hrp = item.enemy, item.hrp
            hitCount += 1
            if not args[1] then
                args[1] = enemy:FindFirstChild("Head") or hrp
            end
            args[2][hitCount] = { [1] = enemy, [2] = hrp }
        end

        if not args[1] or hitCount == 0 then return end
