...
local BringingMobs = {}
local EnemyFolder = workspace:FindFirstChild("Enemies") or workspace.Enemies
local BossSet = {}
for _, bossName in ipairs(BossList) do
    BossSet[bossName] = true
end

local function isBossMob(mob)
    return mob ~= nil and BossSet[mob.Name] == true
end

local MobVisibilityCache = setmetatable({}, {__mode = "k"})

local function HasVisiblePart(mob)
    local cached = MobVisibilityCache[mob]
    ...
    MobVisibilityCache[mob] = visible
    return visible
end
...
local function IsValidMob(v)
    ...
    if not HasVisiblePart(v) then return false end
    return true
end
...
BringEnemy = function()
    ...
    for mobKey in pairs(BringingMobs) do
        ...
        local found = EnemyFolder:FindFirstChild(parts[#parts])
...
task.spawn(function()
  while task.wait(0.1) do
    ...
    Test.Parent = plr.Character
        ...
        for _, no in ipairs(plr.Character:GetDescendants()) do if no:IsA("BasePart") then no.CanCollide = false end end
      else
        shouldTween = false
        getgenv().TweenLockToken = nil
        getgenv().TweenTargetCFrame = nil
  ...
  while task.wait(0.1) do
    if getgenv().AutoMaterial then
      pcall(function()
        ...
        for _, EnemyName in ipairs(MMon) do
          for _, v in pairs(EnemyFolder:GetChildren()) do processEnemy(v, EnemyName) end
        end
        handleEnemySpawns()
      end)
    end
    ...
    pcall(function()
      if _G.AutoSaber and plr.Data.Level.Value >= 200 and not plr.Backpack:FindFirstChild("Saber") and not plr.Character:FindFirstChild("Saber") then
        if workspace.Map.Jungle.Final.Part.Transparency == 0 then
		      ...
		      if plr.Backpack:FindFirstChild("Torch") or plr.Character:FindFirstChild("Torch") then
		        ...
		        if replicated.Remotes.CommF_:InvokeServer("ProQuestProgress","SickMan") ~= 0 then
		          ...
		          elseif replicated.Remotes.CommF_:InvokeServer("ProQuestProgress","RichSon") == 0 then
			        if EnemyFolder:FindFirstChild("Mob Leader") or replicated:FindFirstChild("Mob Leader") then
	     ...
	     if EnemyFolder:FindFirstChild("Saber Expert") or replicated:FindFirstChild("Saber Expert") then
	       for _,v in pairs(EnemyFolder:GetChildren()) do
    ...
    pcall(function()
      ...
      if not GetBP("Dark Blade") then replicated.Remotes.CommF_:InvokeServer("LoadItem","Dark Blade") end
        if GetBP("Fist of Darkness") > 1 then
          if not EnemyFolder:FindFirstChild("Darkbeard") then
...
task.spawn(function()
  while task.wait(0.1) do
    if _G.Auto_Def_DarkCoat then
      pcall(function()
        if GetBP("Fist of Darkness") and not EnemyFolder:FindFirstChild("Darkbeard") then          
          _tp(CFrame.new(3677.08203125, 62.751937866211, -3144.8332519531))
...
task.spawn(function()      
  while task.wait(0.2) do
    pcall(function()
      if _G.obsFarm then
        if World1 then
          if EnemyFolder:FindFirstChild("Galley Captain") then
            if KenTest then
              repeat wait()
                plr.Character.HumanoidRootPart.CFrame = EnemyFolder:FindFirstChild("Galley Captain").HumanoidRootPart.CFrame * CFrame.new(3,0,0)
              until _G.obsFarm == false or KenTest == false
            else
              repeat wait()
                plr.Character.HumanoidRootPart.CFrame = EnemyFolder:FindFirstChild("Galley Captain").HumanoidRootPart.CFrame * CFrame.new(0,50,0)
        ...
        elseif World2 then
          if EnemyFolder:FindFirstChild("Lava Pirate") then
            if KenTest then
              repeat wait()
                plr.Character.HumanoidRootPart.CFrame = EnemyFolder:FindFirstChild("Lava Pirate").HumanoidRootPart.CFrame * CFrame.new(3,0,0)
              until _G.obsFarm == false or KenTest == false
            else
              repeat wait()
                plr.Character.HumanoidRootPart.CFrame = EnemyFolder:FindFirstChild("Lava Pirate").HumanoidRootPart.CFrame * CFrame.new(0,50,0)
              until _G.obsFarm == false or KenTest
            end
          else
        ...
        elseif World3 then
          if EnemyFolder:FindFirstChild("Venomous Assailant") then
            if KenTest then
              repeat wait()
                _tp(EnemyFolder:FindFirstChild("Venomous Assailant").HumanoidRootPart.CFrame * CFrame.new(3,0,0))
              until _G.obsFarm == false or KenTest == false
            else
              repeat wait()
                _tp(EnemyFolder:FindFirstChild("Venomous Assailant").HumanoidRootPart.CFrame * CFrame.new(0,50,0))
              until _G.obsFarm == false or KenTest
            end
          else
...
DragonMobClear=function(a,b,c)if EnemyFolder:FindFirstChild(b)then for d,e in pairs(EnemyFolder:GetChildren())do if e.Name==b and Attack.Alive(e)then if a then Attack.Kill(e,a)end end end else _tp(c)end end
task.spawn(function()
  while task.wait(0.1) do 
...
task.spawn(function()
  while task.wait(0.1) do
    pcall(function()
      if _G.Prehis_Skills then
        if EnemyFolder:FindFirstChild("Lava Golem") then
