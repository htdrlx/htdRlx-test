...
-- Noryx: emergency tween stop
local function NoryxStopTween()
    pcall(function()
        getgenv().TweenLockToken = nil
        getgenv().TweenTargetCFrame = nil
        ...
        if hrp then
            ...
            -- Cancel any active TweenService tweens on the root part.
            pcall(function()
...
_G.ActiveTween = nil
_G.ActiveTweenHRP = nil
_G.ActiveTweenToken = 0
...
_tp = function(I, lockToken)
    ...
    -- Chỉ cho phép một tween điều khiển HRP tại một thời điểm.
    if _G.ActiveTween then
        pcall(function() _G.ActiveTween:Cancel() end)
        _G.ActiveTween = nil
    end

    _G.ActiveTweenToken += 1
    local token = _G.ActiveTweenToken
    _G.ActiveTweenHRP = HRP
    ...
    task.spawn(function()
        ...
        if token ~= _G.ActiveTweenToken then return end

        _G.ActiveTween = nil
        _G.ActiveTweenHRP = nil

        if state == Enum.PlaybackState.Completed and shouldTween then
            if lockToken == nil or getgenv().TweenLockToken == lockToken then
...
local TranslationQueued = {}
local TranslationWorkerRunning = false

local function QueueTranslation(obj)
    ...
    if TranslationQueued[obj] then return end
    TranslationQueued[obj] = true
    TranslationQueue[#TranslationQueue + 1] = obj

    if TranslationWorkerRunning then return end
    TranslationWorkerRunning = true

    task.spawn(function()
        while _G.AutoTranslate and #TranslationQueue > 0 do
            local item = table.remove(TranslationQueue, 1)
            TranslationQueued[item] = nil
            if item and item.Parent then
                local raw = item:GetAttribute("RawText") or item.Text
                local translated = GoogleTranslate(raw)
                if item.Parent and _G.AutoTranslate and item:GetAttribute("RawText") == raw then
...
local function UpdateUI(obj)
    QueueTranslation(obj)
end

local function QueueAllUI()
...
-- Noryx: manual emergency brake for all active tween movement
pcall(function()
    Tabs.Misc:AddButton({
        Name = "⛔ Stop Tween",
        Description = "Hủy tween đang chạy và dừng bay ngay lập tức",
        Callback = function()
            NoryxStopTween()
        end
    })
    ...
    Callback = function(Value)
        ...
        if not _G.TranslationWatcher then
            _G.TranslationWatcher = CoreGui.DescendantAdded:Connect(function(obj)
                task.defer(function()
                    if _G.AutoTranslate then QueueTranslation(obj) end
                end)
            end)
        end
    end
    ...
    Callback = function()
        task.spawn(function()
            pcall(function()
                ...
                -- Bay ổn định: không dùng BodyVelocity vì nó tranh quyền điều khiển với Tween.
                -- Tắt collision một lần trước khi bay thay vì quét GetDescendants() mỗi frame.
                ...
                -- Khôi phục collision sau khi bay xong.
                for part, oldState in pairs(collisionState) do
